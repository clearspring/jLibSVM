package libsvm.svm.kernel.solver;

import libsvm.svm.kernel.QMatrix;
...
public class Solver {
	...
	void reconstruct_gradient()
	{
		...
		if (nr_free*l > 2*active_size*(l-active_size))
		{
			...
			{
				float[] Q_i = qMatrixUtil.get_Q(i,active_size);
				for(j=0;j<active_size;j++)
					if(is_free(j))
						G[i] += alpha[j] * Q_i[j];
			}	
		...
		else
		{
			for(i=0;i<active_size;i++)
				if(is_free(i))
				{
					float[] Q_i = qMatrixUtil.get_Q(i,l);
	...
	public void Solve(int l, QMatrix Q, double[] p_, byte[] y_,
	...
	int select_working_set(int[] working_set)
	{
		...
		if(i != -1) // null Q_i not accessed: Gmax=-INF if i=-1
			Q_i = qMatrixUtil.get_Q(i,active_size);
