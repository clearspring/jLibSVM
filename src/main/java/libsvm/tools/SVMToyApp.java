...
public class SVMToyApp extends Applet {
	...
	void button_run_clicked(String args)
	{
		...
		// build problem
		SVNProblem prob = new SVNProblem();
		...
		else if(param.svm_type == SVMParams.EPSILON_SVR ||
		...
		else
		{
			...
			for (int i = 0; i < XLEN; i++)
				for (int j = 0; j < YLEN ; j++) {
					...
					if (param.svm_type == SVMParams.ONE_CLASS && d<0) d=2;
	...
	{
		new AppletFrame("SVMToyApp",new SVMToyApp(),500,500+50);
	}
}
